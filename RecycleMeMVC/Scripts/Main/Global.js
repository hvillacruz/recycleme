    ...
    this.GetMessage = function () {
        ...
        AjaxNinja.Invoke(ODataApi.Message + "?$filter=ReceiverId eq '" + global.User.UserId() + "'&$orderby=DateSent desc&$expand=Sender", "GET", {}, function (data) {
            ...
            $(data.value).each(function (i, value) {
                ...
                var date = BrowserDetect.browser != "Firefox" && BrowserDetect.browser != "Safari" && BrowserDetect.browser != "IE 11" ? new Date(Date.parse(value.DateSent.replace("T", " "))) : new Date(Date.parse(value.DateSent));
                value.DateSent = date;
                if (date != null)
